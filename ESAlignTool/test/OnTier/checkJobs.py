...
def resubmit( path, job, queue ):
    ...
    if os.path.isdir(path) and 'output' in os.listdir(path) and 'input' in os.listdir(path):
       ...
       # cmd = 'bsub -q '+queue + ' -o ' +path+'/output/job_'+str(job)+'.log'+' source '+path+'/input/job_'+str(job)+'.sh' 
        cmd = 'qsub -q cms -V  -o  ' +path+'/output/job_'+str(job)+'.log'+' '+path+'/input/job_'+str(job)+'.sh' 
        os.system(cmd)
        doneResubmit=True
...
def main():
    ...
    if doOneResubmit or checkOneData:
        ...
        noDoneRoot[fname] = []
        noDoneLogs[fname] = []
        getInfos = storeInfo( f, datasetDir[fname], failedInfo[fname], noDoneRoot[fname], noDoneLogs[fname] )
        rootName = getInfos[0]
